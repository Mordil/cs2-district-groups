...
9. **Overlay rendering works** (Phase 4, verified in-game): `OverlayRenderSystem.GetBuffer(out JobHandle)` from a system at `SystemUpdatePhase.ToolUpdate`, then `Buffer.DrawLine(Color, new Line3.Segment(a, b), width)` per district-boundary segment (positions straight from the district's `Game.Areas.Node` buffer render correctly on terrain). Other confirmed Buffer methods from mod source: `DrawCircle(Color outline, Color fill, float outlineWidth, 0, float2 axis, float3 pos, float diameter)`, `DrawDashedLine(Color, Line3.Segment, width, dashLen, gapLen)`. **No obvious filled-polygon primitive** — fill (user-requested) needs `DrawCustomMesh` investigation or a custom `Mesh` from the district's `Triangle` buffer.
