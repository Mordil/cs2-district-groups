# District Groups — Implementation Plan

> **Status (2026-08-08):** Phases 0 ✅, 1 ✅, 2 ✅ — **M1 PASSED; M2 core PASSED.** Phase 2 verified in-game: groups persist in saves with entity remap; assign/unassign round-trips; exclusive expansion works; ghost-entity lifecycle bug found and fixed (`OnGamePreload` purge + `OnGameLoadingComplete` prune, both observed working in logs — see RESEARCH.md findings #7/#8). Registry API in `DistrictGroupSystem.cs`; components in `DistrictGroupComponents.cs`; test hotkeys Ctrl+Shift+G/U. Mod-removed test also passed: save loads clean without the mod (`Player.log`: "Not serializable type: multi_district_tool.*" — components skipped harmlessly; the namespace is now `DistrictGroups`, see the rebrand note below), vanilla `ServiceDistrict` data survives stripping, and reinstalling recovers un-stripped saves fully. **Phase 2 closed with the full lifecycle matrix verified.**
>
> **Phase 3 ✅ (2026-08-08):** `DistrictGroupSyncSystem` (at `SystemUpdatePhase.Modification5`, `RequireForUpdate` on District+Deleted) prunes deleted districts from groups live and re-expands — verified in-game ("Sync: removed 1 deleted district(s)..."). Defense-in-depth: `ExpandToBuilding` skips dead/deleted members so dangling refs can never reach vanilla buffers. Also observed: vanilla `ServiceDistrictSystem` independently prunes orphaned (unassigned) buildings' buffers; repainted districts don't auto-join groups (by design); assignment to a newly plopped building works. **M2 complete.**
>
> **Phase 4 ✅ outline variant (2026-08-08):** `DistrictGroupOverlaySystem` draws member-district boundaries per group in distinct palette colors via `OverlayRenderSystem.Buffer.DrawLine(Color, Line3.Segment, width)` (signature confirmed compiling + rendering in-game); toggle hotkey Ctrl+Shift+O. Step 4.3 resolved by research — `Game.Objects.Color` is a palette-variation index (bytes), not RGB; it cannot express per-group colors, so no experiment needed.
> **Fill: investigated, built, and reverted by user decision (2026-08-09).** The CPU terrain-conforming mesh approach worked (recipe preserved in RESEARCH.md finding #9b and git history) but was dropped in favor of border-only overlays — terrain-hugging colored outlines, which are cheap (no meshes, no cache, ~<1ms/frame) and visually close to vanilla borders. `ilspycmd` is installed at `%TEMP%\ilspy-tools\` (global install blocked by the `~/.dotnet/tools` symlink; used `--tool-path`), and a decompiled `OverlayRenderSystem` sits in the session scratchpad if needed again.
> **Phase 4 CLOSED (border-only).**
>
> **Phase 5 ✅ (2026-08-09) — M4 ACHIEVED: full workflow usable from in-game UI.** 5.1: UI toolchain scaffolded from BetterBulldozer's template (webpack/React/TS in `UI/`, bundles `district-groups.mjs`, auto-rebuilt by `dotnet build` via `UI/build-ui.bat`; binding round-trip verified). 5.2: group manager panel (top-left "Districts (N)" button → create/rename/retype/delete groups, add/remove members, panel-open drives overlay). 5.3: vanilla info-panel section via `InfoSectionBase` (`DistrictGroupSection` — JS keys off the C# type's FULL NAME) with per-service-type candidate filtering (prefab-data detection; unknown types fall back to Generic = show all) and assign/unassign; registry `Version` counter keeps the section live. All verified in-game via logs. Remaining: Phase 6 (validation/polish/publish) + cosmetics (section header locale, vanilla-styled buttons, per-group color swatches in panel).
>
> **Rebrand (2026-08-09): the mod is now "District Groups".** Working title `multi-district-tool` is gone: assembly/project/mod id → `district-groups`, C# namespace → `DistrictGroups`, `MultiDistrictUISystem` → `DistrictGroupsUISystem`, PDX `DisplayName` → "District Groups". Consequences: the log is now `DistrictGroups.Mod.log`, the settings/keybinding file moved (hotkeys reset to defaults), and **group data in pre-rebrand saves no longer deserializes** because custom `ISerializable` components are keyed by full type name — the `Mod Test` save's groups must be recreated through the Phase 5 UI. Vanilla `ServiceDistrict` buffers were unaffected. Three string pairs stay coupled with no build-time check: mod.json `id` ↔ assembly name, `kBindingGroup` ↔ mod.json `id`, and the `componentList` key ↔ `DistrictGroups.DistrictGroupSection` (see `BUILDING.md`).
>
...
Confirm `CSII_TOOLPATH` is set, `dotnet build` succeeds, the mod deploys to the game's mods folder (the toolchain's Mod.targets does this), and `DistrictGroups.Mod.OnLoad` appears in `Logs/` when the game starts.
