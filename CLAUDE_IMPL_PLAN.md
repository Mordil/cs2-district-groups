...
> **Open follow-up — filled polygons (user wants fill, not just border):** the overlay Buffer exposes no obvious polygon-fill primitive. Investigate (a) `DrawCustomMesh` (name exists in Game.dll metadata — decompile `OverlayRenderSystem.Buffer` to check it and any triangle methods), or (b) building a Unity `Mesh` from the district's `Game.Areas.Triangle` buffer and rendering it directly with a transparent material. Blocker hit when trying to install ilspycmd: a stray **file** named `C:\Users\Nathan\.dotnet\tools` prevents `dotnet tool install` (delete/rename it first, or decompile in Rider instead).
> **Next: fill investigation, then Phase 5 (UI).**
>
